...
export default function BSESwingBullishPage() {
  ...
  // Load BSE swing bullish signals
  const loadBullishSignals = useCallback(async () => {
    try {
      ...
      if (error) {
        console.error("Error loading BSE swing bullish signals:", error);
      } else {
        setSignals(data || []);
        setLastUpdate(new Date());
      }
    } catch (err) {
      console.error("Exception loading BSE swing bullish signals:", err);
    } finally {
      setIsLoading(false);
    }
  }, [supabase]);
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Bullish swing signals from BSE equity stocks (1-15 days)
            </p>
          </div>
        </div>
        <div className="flex items-center gap-3">
        ...
        <CardContent>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-3 text-xs sm:text-sm">
            <div className="flex items-start gap-2">
              <Badge className="bg-green-500/20 text-green-500 mt-1">1</Badge>
              <span className="text-sm">BSE Equity Stocks</span>
            </div>
            <div className="flex items-start gap-2">
              <Badge className="bg-green-500/20 text-green-500 mt-1">2</Badge>
              <span className="text-sm">Trading above 20 EMA (Daily)</span>
            </div>
            <div className="flex items-start gap-2">
              <Badge className="bg-green-500/20 text-green-500 mt-1">3</Badge>
              <span className="text-sm">Trading above 20 EMA (Hourly)</span>
            </div>
            <div className="flex items-start gap-2">
              <Badge className="bg-green-500/20 text-green-500 mt-1">4</Badge>
              <span className="text-sm">Volume &gt; 1.5x average volume</span>
            </div>
            <div className="flex items-start gap-2">
              <Badge className="bg-green-500/20 text-green-500 mt-1">5</Badge>
              <span className="text-sm">Price breakout from consolidation</span>
            </div>
            <div className="flex items-start gap-2">
              <Badge className="bg-green-500/20 text-green-500 mt-1">6</Badge>
              <span className="text-sm">RSI: 50 &lt; RSI &lt; 70</span>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <h3 className="text-lg font-semibold mb-2">No Bullish Signals Found</h3>
