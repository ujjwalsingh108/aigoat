...
/**
 * GET /api/signals/intraday-index
 * 
 * Query params:
...
export async function GET(req: Request) {
  try {
    ...
    // Build cache key
    const cacheKey = buildSignalKey("intraday-index", minutesAgo);
    ...
    console.error("Error fetching intraday index signals:", error);
    return NextResponse.json(
      {
        success: false,
        error: "Failed to fetch intraday index signals",
      },
      { status: 500 }
    );
  }
