...
// Fetch historical data from Kite API (hourly data)
async function fetchSymbolData(
  ...
  const url = `https://api.kite.trade/instruments/historical/${instrumentToken}/60minute`;
...
async function processAllSymbols() {
  ...
  try {
    ...
    // Get all symbols from nse_equity_top_1000_symbols table
    let symbols = await getSymbolsFromDB();

    console.log(`📊 Processing ${symbols.length} NSE equity symbols (hourly data)...`);
  ...
  try {
    await supabase.from("auto_fetch_logs").insert({
      ...
      config: {
        ...
        sourceTable: "nse_equity_top_1000_symbols",
        destinationTable: "historical_prices_nse_swing_hourly",
      },
    });
  } catch (error) {
...
Deno.serve(async (req) => {
  try {
    ...
    return new Response(
      JSON.stringify({
        success: result.success,
        message: `Auto-fetch completed for ${result.totalSymbols} NSE equity stocks (hourly data)`,
        summary: {
