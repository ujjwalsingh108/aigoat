...
// Get all NSE equity symbols from database (2515 stocks)
...
> {
  ...
  if (!data || data.length === 0) {
    throw new Error(
      "No symbols found in kite_nse_equity_symbols table. Please run fetch-nse-equity-stocks.js first."
    );
  }
...
// Process all symbols (2515 NSE equity stocks)
async function processAllSymbols() {
  ...
  try {
    ...
    // Get all symbols from kite_nse_equity_symbols table (2515 stocks)
