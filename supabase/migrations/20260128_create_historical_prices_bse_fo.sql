-- Migration: Create historical_prices_bse_fo table
-- Created: 2026-01-28
-- Purpose: Store 5-minute interval historical price data for BSE F&O instruments
...
CREATE INDEX idx_historical_prices_bse_fo_instrument_type 
  ON public.historical_prices_bse_fo (instrument_type);
...
COMMENT ON COLUMN public.historical_prices_bse_fo.symbol IS 
  'Full BSE F&O symbol (e.g., "SENSEX26FEB75000CE")';

COMMENT ON COLUMN public.historical_prices_bse_fo.underlying IS 
  'Underlying asset (e.g., "SENSEX", "BANKEX")';

COMMENT ON COLUMN public.historical_prices_bse_fo.instrument_type IS 
  'Type of instrument: FUT (Futures), CE (Call Option), PE (Put Option)';

COMMENT ON COLUMN public.historical_prices_bse_fo.expiry IS 
  'Expiry date of the F&O contract';

COMMENT ON COLUMN public.historical_prices_bse_fo.strike IS 
  'Strike price for options (NULL for futures)';

COMMENT ON COLUMN public.historical_prices_bse_fo.timestamp IS 
  'Precise timestamp with timezone for the 5-minute candle';

COMMENT ON COLUMN public.historical_prices_bse_fo.interval_type IS 
