/**
 * Fetch 50+ Days of Hourly Candles for NSE Swing Trading
 * 
 * This script fetches hourly (60min) historical candles for NSE top 1000 symbols
 * and stores them in historical_prices_nse_swing_hourly table.
 * 
...
/**
 * Fetch NSE top 1000 symbols from Supabase
 */
async function fetchNseSymbols(): Promise<Symbol[]> {
  ...
  console.log(`Fetching NSE top 1000 symbols...`);
  ...
  if (error) {
    throw new Error(`Error fetching NSE symbols: ${error.message}`);
  }

  console.log(`✅ Fetched ${data?.length || 0} NSE symbols`);
  return data || [];
}
...
async function processSymbolsBatch(
  ...
  console.log(`\n📊 Processing ${symbols.length} NSE symbols in batches of ${BATCH_SIZE}...`);
  ...
  console.log("\n" + "=".repeat(80));
  console.log("📊 NSE SWING HOURLY DATA FETCH SUMMARY");
...
async function main() {
  try {
    ...
    console.log("\n" + "=".repeat(80));
    console.log("🚀 FETCH NSE SWING HOURLY DATA (50+ DAYS MINIMUM)");
    console.log("=".repeat(80));
    ...
    console.log(`💾 Table: historical_prices_nse_swing_hourly`);
    console.log("=".repeat(80) + "\n");
    ...
    // Fetch NSE symbols
    const nseSymbols = await fetchNseSymbols();
    if (nseSymbols.length > 0) {
      await processSymbolsBatch(kite, nseSymbols);
    } else {
      console.log("⚠️  No NSE symbols found to process");
    }

    console.log("\n" + "=".repeat(80));
    console.log("✅ NSE SWING HOURLY DATA SYNC COMPLETED");
    console.log("=".repeat(80));
    console.log("\n💡 Next steps:");
    console.log("1. Run the swing scanner: pm2 restart nse-swing-positional-scanner");
    console.log("2. Check results in: nse_swing_positional_bullish & nse_swing_positional_bearish tables\n");
...
export { fetchNseSymbols, fetchHourlyCandles, storeHourlyCandles };
