/**
 * 20-Day Historical Data Fetcher for NSE Equity
 * 
 * Purpose: Fetch and store 20 days of 5-minute interval historical data
 * for NSE stocks from nse_bse_equity_top_1000_symbols table.
 * 
 * Data Flow:
 * - Source: nse_bse_equity_top_1000_symbols table (all symbols)
 * - Destination: historical_prices_nse_equity
 * 
 * Features:
 * - 20-day rolling window (auto-cleanup of old data)
 * - Stores in NSE historical table
 * - Rate limiting and retry logic
 * - Batch processing
 * 
 * Usage:
 *   npm run fetch-20day-historical-nse
 * 
...
const CONFIG = {
  ...
  DELAY_BETWEEN_REQUESTS: 500, // 500ms between each request
...
class NseHistoricalDataFetcher {
  ...
  /**
   * Fetch all symbols from unified top 1000 table (no exchange filter)
   * Note: Uses same symbols for NSE data
   */
  async getAllSymbols(): Promise<SymbolData[]> {
    console.log("📊 Fetching symbols from nse_bse_equity_top_1000_symbols...");
    ...
    console.log(`✅ Loaded ${data.length} symbols (using for NSE data)`);
  ...
  /**
   * Save historical data to NSE table
   */
  async saveHistoricalData(rows: HistoricalPriceRow[]): Promise<boolean> {
    ...
    try {
      ...
      if (error) {
        console.error(`   ❌ NSE insert failed:`, error.message);
        return false;
      }

      return true;
    } catch (error: any) {
      console.error(`   ❌ NSE database error:`, error.message);
      return false;
    }
  }
  ...
  async cleanupOldData(): Promise<number> {
    try {
      ...
      console.log(
        `🧹 Cleaning up NSE data older than ${cutoffDate.toISOString().split("T")[0]}...`
      );
      ...
      if (error) {
        console.error(`❌ NSE cleanup failed:`, error.message);
        return 0;
      }

      const deletedCount = data?.length || 0;
      console.log(`✅ NSE: Deleted ${deletedCount.toLocaleString()} old records`);
      return deletedCount;
    } catch (error: any) {
      console.error(`❌ NSE cleanup error:`, error.message);
      return 0;
    }
  }

  /**
   * Process symbols and store in NSE historical table
   */
  async processSymbols(
    symbols: SymbolData[]
  ): Promise<{ success: number; failed: number; totalCandles: number }> {
    console.log(`\n${"=".repeat(60)}`);
    console.log(`📊 Processing NSE Equity (${symbols.length} symbols)`);
   ...
   */
  async execute() {
    console.log("🚀 20-Day NSE Historical Data Fetcher");
    console.log("=".repeat(60));
    ...
    try {
      ...
      // 2. Process and store in NSE historical table
      ...
      console.log("");
      console.log("NSE Historical Data Results:");
...
(async () => {
  const fetcher = new NseHistoricalDataFetcher();
