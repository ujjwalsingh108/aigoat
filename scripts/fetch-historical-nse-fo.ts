/**
 * Zerodha Historical Data Fetcher - NSE F&O
 * 
 * Purpose: Fetch and store 3 months of 5-minute interval historical data
 * for top 1000 NSE F&O instruments into historical_prices_nse_fo table.
 * 
...
const CONFIG = {
  ...
  FOCUS_UNDERLYINGS: ["NIFTY", "BANKNIFTY", "FINNIFTY"], // Can add individual stocks
...
// =================================================================
// ZERODHA NSE F&O HISTORICAL DATA FETCHER
// =================================================================

class ZerodhaNseFoHistoricalFetcher {
  ...
  /**
   * Fetch NSE F&O symbols with filters
   */
  async getNseFoSymbols(): Promise<FoSymbolData[]> {
    ...
    if (error) {
      throw new Error(`Failed to fetch F&O symbols: ${error.message}`);
    }

    console.log(`✅ Loaded ${data.length} NSE F&O instruments (before filtering)`);
  ...
  async execute() {
    console.log("🚀 Starting NSE F&O Historical Data Fetch");
    ...
    console.log(`💾 Destination: historical_prices_nse_fo`);
    console.log("=" .repeat(60));

    try {
      const symbols = await this.getNseFoSymbols();
...
(async () => {
  const fetcher = new ZerodhaNseFoHistoricalFetcher();
