/**
 * Fetch BSE F&O (Futures & Options) Symbols from Zerodha Kite
 * 
 * This script fetches all BSE F&O instruments from Kite Connect API
 * and stores them in the kite_bse_fo_symbols table in Supabase.
 */
 ...
 * Examples: SENSEX24JAN24000CE -> SENSEX, RELIANCE24FEB3000CE -> RELIANCE
 */
function extractUnderlying(tradingSymbol: string, instrumentType: string): string {
  if (instrumentType === "FUT") {
    // For futures: SENSEX24JANFUT -> SENSEX, RELIANCE24FEBFUT -> RELIANCE
    return tradingSymbol.replace(/\d{2}[A-Z]{3}FUT$/i, "");
  } else {
    // For options: SENSEX24JAN24000CE -> SENSEX, RELIANCE24FEB3000CE -> RELIANCE
...
function filterBSEFOSymbols(instruments: KiteInstrument[]): BSEFOSymbol[] {
  ...
  const bseForD = instruments
    .filter((instrument) => {
      // Filter for BFO exchange (BSE F&O segment)
      return (
    ...
    .map((instrument) => {
      ...
      const segment =
        instrument.instrument_type === "FUT" ? "BFO-FUT" : "BFO-OPT";
  ...
  console.log(`✓ Filtered ${bseForD.length} BSE F&O symbols`);

  // Show breakdown
  const futures = bseForD.filter((s) => s.instrument_type === "FUT").length;
  const callOptions = bseForD.filter((s) => s.instrument_type === "CE").length;
  const putOptions = bseForD.filter((s) => s.instrument_type === "PE").length;
...
/**
 * Store BSE F&O symbols in Supabase
 */
async function storeBSEFOSymbolsInSupabase(symbols: BSEFOSymbol[]): Promise<void> {
  ...
  try {
    ...
    console.log("Clearing existing BSE F&O symbols...");
...
async function main() {
  try {
    console.log("\n" + "=".repeat(70));
    console.log("BSE F&O SYMBOLS FETCH & STORE");
    console.log("=".repeat(70) + "\n");

    // Step 1: Fetch all instruments from Kite
    const instruments = await fetchKiteInstruments();

    // Step 2: Filter BSE F&O symbols
    const bseForD = filterBSEFOSymbols(instruments);

    if (bseForD.length === 0) {
      console.log("⚠ No BSE F&O symbols found");
      return false;
    }

    // Step 3: Store in Supabase
    await storeBSEFOSymbolsInSupabase(bseForD);

    console.log("\n" + "=".repeat(70));
    console.log("✓ BSE F&O SYMBOLS SYNC COMPLETED");
    console.log("=".repeat(70));
    console.log(`\n${bseForD.length} BSE F&O symbols are now available in your database`);
    console.log("You can query them from the 'kite_bse_fo_symbols' table");
...
export { fetchKiteInstruments, filterBSEFOSymbols, storeBSEFOSymbolsInSupabase };
