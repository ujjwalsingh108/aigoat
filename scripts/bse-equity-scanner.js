...
// =================================================================
// 📊 BSE EQUITY SCANNER (BATCH MODE)
// =================================================================

class BseEquityScanner {
  constructor() {
  ...
  async initialize() {
    console.log("🚀 Initializing BSE Equity Scanner (Batch Mode)...");
  ...
  async analyzeSymbol(symbolData) {
    ...
    if (!historical || !daily) {
      [historical, daily] = await Promise.all([
        ...
        this.db.getDailyCandles(symbol, 'historical_prices_bse_equity', 365),
      ]);
  ...
  async cleanupStaleSignals() {
    try {
      await this.db.cleanupStaleSignals('bullish_breakout_bse_eq', CONFIG.SIGNAL_TTL_MINUTES);
      await this.db.cleanupStaleSignals('bearish_breakout_bse_eq', CONFIG.SIGNAL_TTL_MINUTES);
  ...
  async start() {
    console.log("🚀 Starting BSE Equity Scanner (Batch Mode)...");
  ...
  stop() {
    ...
    console.log("⏹️ BSE Equity Scanner stopped.");
  }
}
...
const scanner = new BseEquityScanner();
...
// Graceful shutdown
process.on("SIGINT", () => {
  console.log("\n⏹️ Shutting down BSE Equity Scanner...");
  scanner.stop();
  process.exit(0);
});
